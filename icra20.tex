...
%    First, the Head robot is initiailized to a random location followed by N-1 robots with roughly a distance of OO cm between neighbors and their random orientations.
    ...
    We designed a central trajectory planner $\Psi$ to generate highly 
    ...
    denoted as $c_1, c_2, ..., c_{96}$ where each cell is networked with 
    other neighboring cells in $8$~directions. 
	...
	where $A(j,k)$ is the function returning either $True$ if $c_j$ 
	and $c_k$ are adjacent or $False$ otherwise. 
    ...
    the \emph{Head} robot appeared to trigger roughly $3$~times of destination 
