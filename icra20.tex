...
\begin{document}
	...
	We basically follow the problem configurations for remote teammate localization introduced in 
	...
	The robot on one end is the only robot that moves to a 
	...
	positions of their close neighbors. As will be explained in 
	Section~\ref{sec:experiments}, we borrow the computational, sensory capacity of a central 
	computer in experiments, Thus, technically the global positions are used to calculate the next 
	destination of each robot, and then the formula in [] helps generate an actual motion command to 
	operate two wheels according to nonholonomic kinematics. 
