...
%	are submitted as well. 
	...
	Basically, we follow the problem description for remote teammate localization
	first introduced in \cite{CPR17}. A robot team moves in a line formation 
	in which each robot has two adjacent neighbors except for the ones at both ends, 
	which has only one neighbor. 
	...
	from the robot closest to the \emph{Head}.
	Each robot has the same capability of sensor and motion under the same 
	...
	motion rules depending on their roles, as formulated in \cite{CPR17}. This catalyzes the
	 ...
	 has an ability to accelerate fast enough to always keep the neighbors within its 
	 sensory range.
...
%	command to operate two wheels to change its pose according to nonholonomic kinematics. 
	...
	The RTL is to localize all the teammates in the view of 
	...
	have been shared with the \emph{Tail} robot, maybe via global communication,
	but from $t+1$, such information is no longer available causing a need of the
	localization approach only utilizing local observations.	
	This may simulate some realistic scenarios where 
	an unexpected technical issue occurs at a time point disabling any data
	transfer, or the robot team intentionally stops the information sharing entering 
	some regions for security or for saving on the communication cost.
