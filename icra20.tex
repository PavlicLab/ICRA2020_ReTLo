...
%    First, the Head robot is initiailized to a random location followed by N-1 robots with roughly a distance of OO cm between neighbors and their random orientations.
    ...
    waypoints for Head to generate highly arbitrary poses
...
%    destination so that the team of robots can provide 
    ...
    visited by the Head, with the latest at $M[0]$ and the earlier 
    at $M[1]$.     
    As the Head has reached its destination, for calculating the next 
    waypoint, $\Psi$ first determines candidate cells $c_i$ such 
    that $i~\notin~M$ and
    $A(M[0],i)=1$ where $A(j,k)$ is the adjacent 
    matrix of $G$ returning either $1$ if $c_j$ and $c_k$ are adjacent 
    geographically or $0$ otherwise. 
    ...
    the Head robot appeared to trigger $3$~times of destination change
    roughly during the inference time period of $8$~seconds.
